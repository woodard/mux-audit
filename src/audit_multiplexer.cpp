 ...
 *    Glibc does not emit `la_symbind`, `la_pltenter`, or `la_pltexit` events
 * for objects loaded into an auditor's namespace (LM_ID_NEWLM). As a result,
 * the multiplexer cannot broadcast these events between sibling sub-auditors.
 * ==============================================================================
 */
...
struct SymBindKey {
  ...
  bool operator==(const SymBindKey &other) const {
    return refcook == other.refcook && defcook == other.defcook &&
           ndx == other.ndx;
  }
};
...
struct Auditor {
  ...
  Elf64_Addr (*pltenter)(Elf64_Sym *, unsigned int, uintptr_t *, uintptr_t *,
                         ARCH_REGS *, unsigned int *, const char *, long int *);
  unsigned int (*pltexit)(Elf64_Sym *, unsigned int, uintptr_t *, uintptr_t *,
                          const ARCH_REGS *, ARCH_RETVAL *, const char *);
  ...
  bool wants_symbind(uintptr_t *refcook, uintptr_t *defcook) {
    ...
    if (obj_flags.count(refcook) && obj_flags.at(refcook) == 0)
      return false;
    if (obj_flags.count(defcook) && obj_flags.at(defcook) == 0)
      return false;
    return true;
  }
};
...
static std::vector<std::unique_ptr<Auditor>> &get_auditors() {
  static std::vector<std::unique_ptr<Auditor>> *auditors =
      new std::vector<std::unique_ptr<Auditor>>();
  return *auditors;
}
...
// Track the namespace LMID of every cookie (native or synthetic) for
// self-reporting bypass
static std::unordered_map<uintptr_t *, Lmid_t> g_cookie_to_lmid;
static std::mutex g_cookie_lmid_mutex;

// Structural offset calculated at runtime to robustly map uninitialized glibc
// cookies
...
// Structurally corrects uninitialized 0x0 cookies passed by glibc back to their
// link_map
static struct link_map *get_lmap_from_cookie(uintptr_t *cookie) {
  if (!cookie)
    return nullptr;
  if (g_cookie_offset != 0) {
    return reinterpret_cast<struct link_map *>(
        reinterpret_cast<char *>(cookie) - g_cookie_offset);
  }
...
unsigned int la_version(unsigned int version) {
  ...
  if (env_ld_audit) {
    ...
    while (std::getline(ss, token, ':')) {
      if (token == my_path ||
          token.find("audit_multiplexer.so") != std::string::npos) {
        found_myself = true;
        continue;
      }
  ...
  if (current_ld_audit2) {
    ...
    while (std::getline(ss, token, ':')) {
      if (!token.empty())
        final_sub_auditors.push_back(token);
    }
  }
  ...
  if (required_tls > 0) {
    ...
    if (env_tunables)
      existing_tunables = env_tunables;
    ...
    if (pos != std::string::npos) {
      ...
      std::string val_str =
          existing_tunables.substr(val_start, val_end - val_start);
      try {
    ...
    if (current_tls_val < required_tls) {
      ...
      if (pos != std::string::npos) {
        ...
        if (val_end != std::string::npos) {
          new_tunables = existing_tunables.substr(0, pos) +
                         existing_tunables.substr(val_end + 1);
        } else {
          new_tunables = (pos > 0 && existing_tunables[pos - 1] == ':')
                             ? existing_tunables.substr(0, pos - 1)
                             : existing_tunables.substr(0, pos);
        }
      } else {
        new_tunables = existing_tunables;
      }
      if (!new_tunables.empty() && new_tunables.back() != ':')
        new_tunables += ":";
      new_tunables +=
          "glibc.rtld.optional_static_tls=" + std::to_string(required_tls);
    }
  }
  ...
  if (requires_reexec) {
    ...
    if (uncontrolled_auditors) {
      fprintf(stderr,
              "[audit_multiplexer] WARNING: Uncontrolled auditors detected. "
              "Re-configuring environment and re-executing...\n");
    }

    if (tls_needs_update) {
      fprintf(stderr,
              "audit_multiplexer: reexecing to acquire more static TLS.\n");
    ...
    for (const auto &aud : prior_auditors) {
      fprintf(stderr,
              "[audit_multiplexer] Moving prior auditor to LD_AUDIT2: %s\n",
              aud.c_str());
      new_ld_audit2 += aud + ":";
    }
    ...
    for (const auto &aud : subsequent_auditors) {
      fprintf(stderr,
              "[audit_multiplexer] Moving subsequent LD_AUDIT entry to "
              "LD_AUDIT2: %s\n",
              aud.c_str());
      new_ld_audit2 += aud + ":";
    }
    ...
    // MUST unset the lock so the newly executed process can initialize the
    // multiplexer
  ...
  for (const auto &lib : audit2_libs) {
    ...
    if (handle) {
      ...
      if (v_func && v_func(LAV_CURRENT) == LAV_CURRENT) {
        ...
        aud->version = v_func;
        aud->objsearch =
            (decltype(aud->objsearch))dlsym(handle, "la_objsearch");
        ...
        aud->symbind64 =
            (decltype(aud->symbind64))dlsym(handle, "la_symbind64");
      ...
      // Fallback strategy: If this is the ONLY sub-auditor and it failed to
      // load, step out of the way entirely and let the application run
      // natively.
      if (audit2_libs.size() == 1) {
        fprintf(stderr, "[audit_multiplexer] Single sub-auditor failed to "
                        "load. Unsetting LD_AUDIT and re-executing...\n");
        unsetenv("LD_AUDIT");
        unsetenv("LD_AUDIT2");
        unsetenv("AM_MUX_ACTIVE");
...
unsigned int la_objopen(struct link_map *map, Lmid_t lmid, uintptr_t *cookie) {
  // Capture structural offset to bypass glibc 0x0 initialization bugs during
  // teardown
  if (g_cookie_offset == 0 && map && cookie) {
    g_cookie_offset =
        reinterpret_cast<char *>(cookie) - reinterpret_cast<char *>(map);
  }
  ...
  if (is_new_ns && ns_cookie) {
    if (am_mark_ns_added(ns_cookie)) {
      for (auto &aud_ptr : get_auditors()) {
        if (aud_ptr->activity)
          aud_ptr->activity(ns_cookie, LA_ACT_ADD);
      }
    }
  }
...
void la_preinit(uintptr_t *cookie) {
  // Safely check the main executable's dynamic section for foreign DT_AUDIT
  // tags
      ...
      for (ElfW(Dyn) *d = dyn; d->d_tag != DT_NULL; ++d) {
        if (d->d_tag == DT_AUDIT || d->d_tag == 0x7ffffffb) {
          ...
          if (strstr(audit_lib, "audit_multiplexer") == nullptr) {
            fprintf(stderr,
                    "[audit_multiplexer] WARNING: Foreign DT_AUDIT directive "
                    "detected: %s\n",
                    audit_lib);
            fprintf(stderr, "  This auditor will load normally but bypasses "
                            "the multiplexer's control.\n");
            fprintf(stderr, "  Suggestion (Option A): Use patchelf to rewrite "
                            "the executable:\n");
            fprintf(stderr, "    patchelf --remove-audit %s <executable>\n",
                    audit_lib);
            fprintf(stderr, "    patchelf --add-audit "
                            "/path/to/audit_multiplexer.so <executable>\n");
          }
        }
      }
    }
  ...
  // Append any pre-existing LM_ID_BASE objects (like audit_multiplexer itself)
  // to g_history
  struct link_map *base_map = _r_debug.r_map;
  while (base_map && base_map->l_prev)
    base_map = base_map->l_prev;
      ...
      for (auto &aud_ptr : get_auditors()) {
        ...
        if (aud_ptr->objopen) {
          unsigned int sub_flags =
              aud_ptr->objopen(base_map, LM_ID_BASE, synthetic_cookie);
  ...
  // Cross-pollinate sub-auditor namespaces (LM_ID_NEWLM) so they see each
  // other.
    ...
    if (dlinfo(aud_ptr->handle, RTLD_DI_LMID, &ns_lmid) == 0 &&
        ...
        if (!is_tracked) {
          ...
          if (is_new_ns) {
            ...
            for (size_t j = 0; j < get_auditors().size(); ++j) {
              if (i == j)
                continue; // Auditor should NOT see itself
              auto &other_aud = get_auditors()[j];
              if (other_aud->activity)
                other_aud->activity(ns_cookie, LA_ACT_ADD);
            }
          }
          ...
          for (size_t j = 0; j < get_auditors().size(); ++j) {
            if (i == j)
              continue; // Auditor should NOT see itself
            auto &other_aud = get_auditors()[j];
            if (other_aud->objsearch)
              other_aud->objsearch(iter->l_name, obj_cookie, LA_SER_AUDIT);
            if (other_aud->objopen) {
              unsigned int sub_flags =
                  other_aud->objopen(iter, ns_lmid, obj_cookie);
      ...
      if (announced_add) {
        for (size_t j = 0; j < get_auditors().size(); ++j) {
          ...
          auto &other_aud = get_auditors()[j];
          if (other_aud->activity)
            other_aud->activity(ns_cookie, LA_ACT_CONSISTENT);
        }
...
void la_activity(uintptr_t *cookie, unsigned int flag) {
  ...
  // duplicate.
  if (flag == LA_ACT_ADD) {
    if (!am_mark_ns_added(effective_cookie))
      return;
  } else if (flag == LA_ACT_DELETE) {
    if (!am_mark_ns_deleting(effective_cookie))
      return;
  ...
  for (auto &aud_ptr : get_auditors()) {
    ...
    if (act_lmid != -1 && act_lmid == aud_lmid)
      continue;
...
unsigned int la_objclose(uintptr_t *cookie) {
  ...
  if (ns_cookie && am_mark_ns_deleting(ns_cookie)) {
    for (auto &aud_ptr : get_auditors()) {
      Lmid_t aud_lmid = -1;
      dlinfo(aud_ptr->handle, RTLD_DI_LMID, &aud_lmid);
      if (close_lmid != -1 && close_lmid == aud_lmid)
        continue;

      if (aud_ptr->activity)
        aud_ptr->activity(ns_cookie, LA_ACT_DELETE);
    }
  }
...
uintptr_t la_symbind64(Elf64_Sym *sym, unsigned int ndx, uintptr_t *refcook,
  ...
  for (auto &aud_ptr : get_auditors()) {
    auto &aud = *aud_ptr;
    if (!aud.symbind64 || !aud.wants_symbind(refcook, defcook))
      continue;
    ...
    uintptr_t ret =
        aud.symbind64(&current_sym, ndx, refcook, defcook, &sub_flags, symname);
    ...
    if (!(sub_flags & LA_SYMB_NOPLTENTER))
      all_nopltenter = false;
    if (!(sub_flags & LA_SYMB_NOPLTEXIT))
      all_nopltexit = false;
  }

  if (any_called) {
    if (all_nopltenter)
      final_flags |= LA_SYMB_NOPLTENTER;
    if (all_nopltexit)
      final_flags |= LA_SYMB_NOPLTEXIT;
    *flags = final_flags;
  }
...
Elf64_Addr LA_PLTENTER_FUNC(Elf64_Sym *sym, unsigned int ndx,
                            uintptr_t *refcook, uintptr_t *defcook,
                            ARCH_REGS *regs, unsigned int *flags,
  ...
  for (auto &aud_ptr : get_auditors()) {
    auto &aud = *aud_ptr;
    if (!aud.pltenter || !aud.wants_symbind(refcook, defcook))
      continue;
    ...
    if (current_sub_flags & LA_SYMB_NOPLTENTER)
      continue;
    any_called = true;
    ...
    Elf64_Addr ret = aud.pltenter(&current_sym, ndx, refcook, defcook, regs,
                                  &current_sub_flags, symname, framesizep);
    ...
    if (!(current_sub_flags & LA_SYMB_NOPLTENTER))
      all_nopltenter = false;
    if (!(current_sub_flags & LA_SYMB_NOPLTEXIT))
      all_nopltexit = false;
  }
...
unsigned int LA_PLTEXIT_FUNC(Elf64_Sym *sym, unsigned int ndx,
                             uintptr_t *refcook, uintptr_t *defcook,
                             const ARCH_REGS *inregs, ARCH_RETVAL *outregs,
                             const char *symname) {
  ...
  for (auto &aud_ptr : get_auditors()) {
    auto &aud = *aud_ptr;
    if (!aud.pltexit || !aud.wants_symbind(refcook, defcook))
      continue;

    {
      std::shared_lock<std::shared_mutex> lock(aud.sym_mutex);
      auto it = aud.sym_flags.find(key);
      if (it != aud.sym_flags.end() && (it->second & LA_SYMB_NOPLTEXIT))
        continue;
    }
    final_ret =
        aud.pltexit(sym, ndx, refcook, defcook, inregs, outregs, symname);
  }
  return final_ret;
}
