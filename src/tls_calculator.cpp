...
static std::string expand_origin(std::string path, const std::string &origin) {
  size_t pos;
  while ((pos = path.find("$ORIGIN")) != std::string::npos)
    path.replace(pos, 7, origin);
  while ((pos = path.find("${ORIGIN}")) != std::string::npos)
    path.replace(pos, 9, origin);
  return path;
}

static std::string search_path_list(const std::string &path_list,
...
static std::string resolve_dependency_path(const std::string &dep_name,
                                           const std::string &loading_obj_path,
                                           const std::string &rpath,
                                           const std::string &runpath) {
  std::string origin = ".";
  size_t slash_pos = loading_obj_path.find_last_of('/');
  if (slash_pos != std::string::npos)
    origin = loading_obj_path.substr(0, slash_pos);
  else if (!loading_obj_path.empty())
    origin = ".";
  ...
  std::vector<std::string> default_paths = {"/lib/x86_64-linux-gnu/",
...
static TlsInfo get_elf_tls_info(const std::string &filepath,
                                std::vector<std::string> &out_dependencies) {
  ...
  while ((scn = elf_nextscn(elf, scn)) != NULL) {
    ...
    if (shdr.sh_type == SHT_DYNAMIC) {
      ...
      for (size_t i = 0; i < dyn_entries; i++) {
        ...
        if (dyn.d_tag == DT_FLAGS && (dyn.d_un.d_val & DF_STATIC_TLS))
          info.requires_ie = true;
        else if (dyn.d_tag == DT_NEEDED) {
          const char *dep_name = elf_strptr(elf, shdr.sh_link, dyn.d_un.d_val);
          if (dep_name)
            out_dependencies.push_back(dep_name);
  ...
  GElf_Ehdr ehdr;
  if (gelf_getehdr(elf, &ehdr) && ehdr.e_type == ET_EXEC)
    info.requires_ie = true;
...
size_t calculate_ie_tls(const std::vector<std::string> &audit_libs,
  ...
  while (!to_process.empty()) {
    ...
    if (visited.find(current_lib) != visited.end())
      continue;
    ...
    // process startup, regardless of whether it explicitly mandates
    // DF_STATIC_TLS.
    if (tls_info.size > 0) {
      dl_tls_static_align = std::max(dl_tls_static_align, tls_info.align);
      dl_tls_static_size =
          roundup(dl_tls_static_size, tls_info.align) + tls_info.size;
    }

    for (const auto &dep : dependencies) {
      std::string full_path = resolve_dependency_path(
          dep, current_lib, tls_info.rpath, tls_info.runpath);
