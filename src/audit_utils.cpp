...
void am_iterate_maps(void (*cb)(struct link_map *)) {
  if (_r_debug.r_version < 2) {
    fprintf(stderr, "[audit_utils] Error: System dynamic linker does not "
                    "support r_version >= 2.\n");
    return;
  }
  struct r_debug_extended *ext_debug =
      reinterpret_cast<struct r_debug_extended *>(&_r_debug);
  ...
  if (it != g_obj_to_ns_cookie.end()) {
    ...
    if (ref_it != g_ns_refcount.end()) {
      ...
      if (ref_it->second == 0) {
        ...
        for (auto ns_it = g_lmid_to_ns_cookie.begin();
             ns_it != g_lmid_to_ns_cookie.end();) {
...
static std::vector<am_init_fini_fn_t> am_get_init_fini(struct link_map *map,
                                                       bool is_init) {
