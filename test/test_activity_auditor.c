...
static unsigned int current_state =
    0; // 0 = unknown/consistent, 1 = ADD, 2 = DELETE
...
unsigned int la_objopen(struct link_map *map, Lmid_t lmid, uintptr_t *cookie) {
  if (current_state != LA_ACT_ADD) {
    fprintf(stderr,
            "[%s] FATAL: la_objopen called but state is NOT LA_ACT_ADD "
            "(state=%u)\n",
...
void la_activity(uintptr_t *cookie, unsigned int flag) {
  current_state = flag;
  const char *act = flag == LA_ACT_ADD
                        ? "ADD"
                        : (flag == LA_ACT_DELETE ? "DELETE" : "CONSISTENT");
...
unsigned int la_objclose(uintptr_t *cookie) {
  if (current_state != LA_ACT_DELETE) {
    fprintf(stderr,
            "[%s] FATAL: la_objclose called but state is NOT LA_ACT_DELETE "
            "(state=%u)\n",
            my_name, current_state);
  }
