...
void la_activity(uintptr_t *cookie, unsigned int flag) {
  ...
  // Extract the link_map pointer from the glibc-provided cookie
  struct link_map *m = (struct link_map *)*cookie;

  // The base namespace (LM_ID_BASE) is always the very first namespace
  // presented to the auditor. If base_head is uninitialized, we can safely
  // initialize it here during the early LA_ACT_ADD.
  if (base_head == NULL && m != NULL) {
    for (base_head = m; base_head->l_prev != NULL;
         base_head = base_head->l_prev)
      ;
  }
  ...
  if (m == base_head) {
    ...
    printf("  la_activity(%p, LA_ACT_%s)\n", cookie,
...
unsigned int la_objopen(struct link_map *m, Lmid_t lmid, uintptr_t *cookie) {
  // This remains intact to support native glibc execution where la_objopen
  // arrives first
  if (lmid == LM_ID_BASE) {
    for (base_head = m; base_head->l_prev != NULL;
         base_head = base_head->l_prev)
  ...
  if (state == LA_ACT_ADD)
    ...
    printf("FAIL. (in state %s)\n", state == LA_ACT_CONSISTENT
                                        ? "LA_ACT_CONSISTENT"
                                    : state == LA_ACT_DELETE ? "LA_ACT_DELETE"
                                                             : "[initial]");
