...
void la_activity(uintptr_t* cookie, unsigned int flag) {
  ...
  // The base namespace (LM_ID_BASE) is always the very first namespace presented to the auditor.
  // If base_head is uninitialized, we can safely initialize it here during the early LA_ACT_ADD.
...
unsigned int la_objopen(struct link_map* m, Lmid_t lmid, uintptr_t* cookie) {
  // This remains intact to support native glibc execution where la_objopen arrives first
  if (lmid == LM_ID_BASE) {
    for (base_head = m; base_head->l_prev != NULL; base_head = base_head->l_prev);
  }
